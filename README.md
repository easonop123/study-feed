...
- **Shareable cards:** clearing the due feed, or earning an Excellence on a written answer, offers a 1080×1920 PNG built for a story. Drawn on a canvas (`drawShareCard`), not screenshotted — html2canvas is a dependency the Artifact build can't take, and a phone-width screenshot is the wrong shape anyway. **The question and the answer are deliberately not on it**: questions are generated from teachers' slides and past papers, and the answer is the student's own work. Content sits between y=800 and y=1660 because Instagram and Snapchat overlay their own furniture on the top and bottom ~250px, and the footer URL is the whole point. Sharing goes through `navigator.share({files})` — there is no API to post to a story directly, so the OS sheet does it — and falls back to a download everywhere else, including the Artifact.
